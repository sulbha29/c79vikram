...
export default class LoginScreen extends Component{
    ...
    signup=(emailid,password,confirmpassword)=>{
        ...
        firebase.auth().createUserWithEmailAndPassword(emailid,password).then((Response)=>{
            return(
                Alert.alert("User added successfully",[{text:"ok",onPress:()=>{this.setState({ismodalvisible:false})}}])
            )
        })
        .catch((error)=>{
            var errorcode=error.code;
        ...
        db.collection("user").add({firstname:this.state.firstname,lastname:this.state.lastname,contact:this.state.contact,emailid:this.state.emailid,address:this.state.emailid})
    }
...
const styles = StyleSheet.create({ container:{ flex:1, backgroundColor:'#F8BE85', alignItems: 'center',
 justifyContent: 'center' }, profileContainer:{ flex:1, justifyContent:'center', alignItems:'center', },
  title :{ fontSize:65, fontWeight:'300', paddingBottom:30, color : '#ff3d00' },
   loginBox:{ width: 300, height: 40, borderBottomWidth: 1.5,
     borderColor : '#ff8a65', fontSize: 20, margin:10, paddingLeft:10 },
...
modalContainer:{ flex:1, borderRadius:20, justifyContent:'center', 
alignItems:'center', backgroundColor:"#ffff", marginRight:30, marginLeft : 30, marginTop:80, marginBottom:80, }, 
